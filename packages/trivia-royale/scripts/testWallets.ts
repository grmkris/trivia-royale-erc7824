...
// Create shared key manager for all test wallets (persistent across test runs)
const keyManager = createFileSystemKeyManager('./test-data');

/**
 * Load all wallets from MNEMONIC in .env
 * Uses FileSystemKeyManager for persistent session keys across test runs
 */
export function loadWallets(): Wallets {
  const mnemonic = testEnv.MNEMONIC;
